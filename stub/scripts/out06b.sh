...
prefix=${1?"*** prefix"}
web_prefix='https://github.com/teemtee/tmt/blob/main'
while read line; do
	yaml_file=${line#tmt/}
	yaml_file_path=$prefix/$yaml_file
	grep "hosts:" $yaml_file_path > /dev/null 2>&1
	(( $? == 0 )) && echo '* [x]' "[$yaml_file]($web_prefix/$yaml_file)"
done < $yaml_files
